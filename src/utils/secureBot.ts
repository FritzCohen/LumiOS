...
import { File, Directory, SystemProps } from "./types";
        ...
        while (queue.length) {
            ...
            for (const char in node) {
                if (char !== 'fail' && char !== 'isEndOfKeyword') {
                    const fail = node.fail[char] || this.root;
                    node[char].fail = fail;
...
export const secureBot: SecureBot = new (class SecureBot {
    ...
    private async init(): Promise<void> {
        ...
        const file = await virtualFS.readfile("System/Plugins/", "SystemProps");
        const content: SystemProps = file.content;
        const canRun = !content.runSecureBot;
        this.ran = canRun;
    }
    ...
    */
    public async checkFiles(): Promise<boolean> {
        if (this.ran) return true;

        const root = virtualFS.getFileSystem();
        if (!root || root.type !== "directory") {
            throw new Error("Invalid file system root.");
        }
