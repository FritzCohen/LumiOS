...
export async function installApp(files: FileList, opts: InstallOptions) {
	const { name, userDirectory, uploadAsApp } = opts;
...
async function handleSingle(
  ...
  // Get filename stuff AFTER. Bug if filename looks like "example (3)"
  const filename = await virtualFS.getUniqueName(basePath, file.name);
  const ext = filename.split(".").pop() || ""; 
  ...
  // Create shortcut payload (same format as folder installs)
  const shortcutPayload = {
    name: filename, // entry file
    path: basePath, // folder only
    icon: icon ? await fileToDataUrl(icon) : null,
  };

  console.log(shortcutPayload);
  

  const shortcutFileName = `${name}.shortcut`;
...
export async function processIndexFile(
  ...
  const { name: appName, pinned, shortcut, uploadAsApp, userDirectory, icon } = opts;
