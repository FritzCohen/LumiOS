...
const Apps = () => {
	...
	const smartSetItems = useCallback(
		(
			...
			const old =
				type === "desktop"
					? previousDesktopItemsRef.current
					: previousTaskbarItemsRef.current;
	...
	useFolderWatcher(`${userDirectory}/Taskbar/`, (items) =>
		smartSetItems("taskbar", items)
	);
	useFolderWatcher(`${userDirectory}/Desktop/`, (items) =>
		smartSetItems("desktop", items)
	);
	...
	const pin = async () => {
		...
		if (
			taskbarItems[
				selectedApp.config.name || selectedApp.config.displayName
			]
		) {
			...
			if (selectedApp.config?.shortcut) {
				await virtualFS.writeFile(
					`${userDirectory}/Taskbar/`,
					selectedApp.config.name,
					{
						name: selectedApp.config.name,
						path: `${userDirectory}/Apps/`,
						icon: selectedApp.config.icon
					},
					"shortcut"
				);	
			} else {
				...
				<>
					<div className="w-full flex justify-between items-center px-5">
						...
						<div className="flex flex-row gap-2 items-center justify-center">
							{!selectedApp.config
								.icon ? undefined : typeof selectedApp.config
									.icon === "string" ? (
								...
								selectedApp.config.icon
									...
									<div
										...
										dangerouslySetInnerHTML={{
											__html: selectedApp.config.icon,
										}}
									/>
								) : (
									<img
					...
					<div className="flex-grow flex flex-col gap-2 my-5 h-full">
						...
						{(() => {
							const appName =
								selectedApp.config.name ||
								selectedApp.config.displayName;
				...
				<>
					{apps.map((app, index) => (
						<div
							...
							className="flex flex-row justify-between items-center bg-surface hover:bg-surfaceAlt rounded-md px-4 py-2 transition-all shadow hover:shadow-md duration-200  cursor-pointer"
						>
							<div className="flex justify-center items-center gap-4">
								{!app.config.icon ? undefined : typeof app
										.config.icon === "string" ? (
									// Check if the string starts with "<svg" or "<img"
									app.config.icon.trim().startsWith("<svg") ||
									app.config.icon
										.trim()
										.startsWith("<img") ? (
										<div
