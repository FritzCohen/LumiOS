...
const Home: React.FC<{ store: useAppstoreProps }> = ({ store }) => {
	return (
		<div className="w-full h-full overflow-y-auto overflow-hidden relative pb-20">
			...
			/>
			<Carousel
				games={randomizeArray(10, store.games)}
				setSelectedGame={store.selectGame}
			/>
			<div className="w-full overflow-y-auto overflow-hidden relative flex justify-center mt-10">
				<Button onClick={() => store.selectGenre("All")}>
					See all games
				</Button>
			</div>{" "}
		</div>
