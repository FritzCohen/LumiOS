...
const HTMLViewer: React.FC<{
  name: string;
  path: string;
  props?: Record<string, any>; // renamed from props
}> = ({ name, path, props: vals = {} }) => {
  const [iframeSrc, setIframeSrc] = useState<string | null>(null);
  const iframeRef = useRef<HTMLIFrameElement>(null);

  // Send vals to iframe on load or when they change
  const postVals = () => {
    if (iframeRef.current?.contentWindow) {
      iframeRef.current.contentWindow.postMessage({ type: 'updateVals', vals }, '*');
    }
  };
  ...
  const injectRuntimeBridge = (html: string) => {
    const bridge = `
      <script>
        window.API = {
          vals: {},
          onValsUpdate(newVals) {
            Object.assign(this.vals, newVals);
            document.dispatchEvent(new CustomEvent('valsUpdated', { detail: newVals }));
          }
        };

        window.addEventListener('message', (e) => {
          if (e.data?.type === 'updateVals') {
            window.API.onValsUpdate(e.data.vals);
          }
        });
      </script>
    `;
  ...
  useEffect(() => {
    const fetchContent = async () => {
      ...
      const rewritten = injectRuntimeBridge(rewriteHtml(target.content as string, urlMap, path));
