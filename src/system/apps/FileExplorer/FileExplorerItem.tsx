...
import OpenWithPopup from "../../gui/components/Popups/OpenWithPopup";
...
const FileExplorerItem: React.FC<FileExplorerItemProps> = ({
  ...
  useEffect(() => {
	...
	const handleOpenWith = (path: string) => {
	...
	const handlePropertiesOpen = (file: NamedFile | NamedDirectory, path: string) => {
		openApp({
			config: {
				name: `${file.name}Properties`,
				displayName: `${file.name} Properties`,
				permissions: 1,
				icon: NotepadIcon,
			},
			mainComponent: (props) => (
		...
		file.type === "directory"
			? [
					...
					},
					{
						name: "Properties",
						icon: faWrench,
						action: () => handlePropertiesOpen(file, directory)
					}
			  ]
			: [
					{
					...
					},
					{
						name: "Open With",
						icon: faArrowUpRightFromSquare,
						action: () => handleOpenWith(directory)
					},
					{
						name: "Copy",
						icon: faCopy,
