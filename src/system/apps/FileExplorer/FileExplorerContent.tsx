...
import { faArrowLeft, faArrowUpRightFromSquare, faClipboard, faCopy, faFileCirclePlus, faFolderPlus, faWrench } from "@fortawesome/free-solid-svg-icons";
...
import ItemInfoPopup from "../../gui/components/Popups/ItemInfoPopup";
...
const FileExplorerContent: React.FC<{
  ...
  const handlePropertiesOpen = async (path: string) => {
    // Get the item name and new path from og path, like "Documents/" becomes name: "Documents", path: ""
    const normalized = path.endsWith("/") ? path.slice(0, -1) : path;
    const lastSlash = normalized.lastIndexOf("/");
    const name = lastSlash === -1 ? normalized : normalized.slice(lastSlash + 1);
    const newPath = lastSlash <= 0 ? "" : normalized.slice(0, lastSlash + 1);
    
    // Get the item from new path and make it into "NamedItem" type 
    const dir = await virtualFS.readdir(newPath);
    const rawFile = dir[name];
    const file = { ...rawFile, name, fullpath: newPath };

    // This is the most amount of comments ur getting from me
    openApp({
      config: {
        name: `${name} Properties`,
        displayName: `${name} Properties`,
        permissions: 1,
        icon: NotepadIcon,
      },
  ...
  // Context menu
      ...
      showContextMenu(e, [
        ...
        },
        {
          name: "Properties",
          icon: faWrench,
          action: () => handlePropertiesOpen(directory)
        }
      ], containerRef);
