...
const Topbar: React.FC = () => {
	...
	const { logout, currentUser, modifyUserProp } = useUser();
	...
	useEffect(() => {
		addMenu({
			...
			icon: logo,
			items: [
			{ label: "About", onClick: () => window.location.href = "https://github.com/FritzCohen/LumiOS" },
	...
	return (
		<div
			...
			className={`flex items-center gap-2 relative 
				${!currentUser?.uiStyle.topbar.visible ? "pointer-events-none invisible" : ""}
				${
			...
			onContextMenu={(e) =>
				showContextMenu(
					...
					[
						{
							name: currentUser?.uiStyle.topbar.visible ? "Hide" : "Show",
							icon: currentUser?.uiStyle.topbar.visible
								? faEyeSlash
								: faEye,
							action: () =>
								modifyUserProp(
						...
						{
							name: currentUser?.uiStyle.topbar.onHover
								? "Show on mouseover"
								: "Show always",
							action: () =>
								modifyUserProp(
