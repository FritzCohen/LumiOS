...
const DesktopItems = () => {
	...
	// Properties tab
	const handlePropertiesOpen = async () => {
		const normalized = desktop.directory.endsWith("/") ? desktop.directory.slice(0, -1) : desktop.directory;
		const lastSlash = normalized.lastIndexOf("/");
		const newPath = lastSlash <= 0 ? "" : normalized.slice(0, lastSlash + 1);

		// Name is constant and known

		// TODO: Make code readable and make special paths reserved names or something????
		const name = "Desktop";
