...
const Taskbar = () => {
	...
	const { userDirectory, currentUser, modifyUserProp } = useUser();
	const {
		contextMenuPosition,
		contextMenuVisible,
		showContextMenu,
	...
	return (
		<div
			...
			style={{
				width: "100%",
				minWidth: currentUser?.uiStyle.taskbar.mode === "full" ? "49%" : "unset",
				borderRadius:
					 currentUser?.uiStyle.taskbar.mode === "full" ? "0px" : "10px",
				marginBottom:
					 currentUser?.uiStyle.taskbar.mode === "full" ? "0px" : "16px",
				display: "flex",
				justifyContent:
					 currentUser?.uiStyle.taskbar.mode === "full"
						? "flex-start"
						: "center",
			}}
		>
				...
				onContextMenu={(e) =>
					showContextMenu(
						...
						[
							...
							{
								name: `Set ${
									 currentUser?.uiStyle.taskbar.mode === "full" ? "Floating" : "Full"
								}`,
								action: () => {
									if (!currentUser) return;

									const mode = currentUser?.uiStyle.taskbar.mode === "full" ? "floating" : "full";
							...
							},
							{
								...
								icon: !currentUser?.uiStyle.taskbar.onHover
									? faEye
									: faEyeSlash,
								action() {
									if (!currentUser) return;
