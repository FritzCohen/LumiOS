...
interface OpenWithProps {
	...
	itemToPass: NamedFile | NamedDirectory;
	path?: string
	fileName?: string
}

/**
 * Opens the window with a target app
 * 
 * Don't use this directly, use the hook instead.
*/
const OpenWithPopup: React.FC<OpenWithProps> = ({ props, itemToPass, path, fileName }) => {
	...
	// Confirm the users selction if they have a "good" selected app
	const handleConfirm = () => {
		...
		<Popup app={props} width={250} height={400} frozenBackground>
			<div className="flex flex-col h-full w-full py-2">
				<div className="flex-grow overflow-auto space-y-2 px-2">
					...
					{/* 
					This is for when the user selects an executable, but backs out later.
					The user can still select it after selecting something else
					*/}
					{customExecutable && (
						<div
							className={`w-full flex gap-2 items-center p-2 file-popup-item cursor-pointer rounded ${
