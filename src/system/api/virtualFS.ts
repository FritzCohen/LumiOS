...
export class VirtualFS {
	...
	public async setRoot(root: Directory) {
		this.root = root;
	...
	/**
	 *
	 * @deprecated Replaced by updateItem
	 *
	...
	public async updateItem<
		...
		if (!item) throw new Error(`Item ${name} does not exist at ${path}`);

		// Validate file type if specified
		if (
			options?.fileType &&
			item.type === "file" &&
			item.fileType !== options.fileType
		) {
			throw createError(FileErrorType.InvalidFileType, {
				extra: `Expecting ${options.fileType} got ${item.fileType}`,
			});
		}

		// Traverse the dot path and set the value
		const keys = (property as string).split(".");
		let target: any = item;
		for (let i = 0; i < keys.length - 1; i++) {
			const key = keys[i];
			if (!(key in target) || typeof target[key] !== "object") {
				target[key] = {}; // create nested object if missing
			}
			target = target[key];
		}
		target[keys[keys.length - 1]] = value;

		// Update modification date if its unspecified by the user
		if (property !== "date") item.date = new Date();
	...
	/**
	 * @deprecated Replaced by deleteItem
	 * 
	 * @param path 
	 * @param fileName 
	 */
	...
	/**
	 * Updated version of deleteFile
	 * 
	 * Needs perms to do certain actions
	 * 
	 * @param path Path to the item
	 * @param itemName Name of the item (Can be file or directory)
	 * @param permission Current permisssion of the user
	 */
	public async deleteItem(path: string, itemName: string, permission?: Permission): Promise<void> {
		const currentDir = await this.getNode(path);
		const item = currentDir.children[itemName];

		if (permission && item.permission > permission) 
			throw createError(FileErrorType.PermissionDenied, { permissionRequired: item.permission });

		try {
			delete currentDir.children[itemName];			
		} catch (error) {
			throw createError(FileErrorType.FileNotFound, { extra: error as unknown as string });
		}
		...
		if (item.type === "directory") {
			...
			await Promise.all(
				Object.entries(item.children).map(([key, child]) =>
					this.copyItem(childPath, key, child)
				)
			);
		} else {
		...
		if (path === "/" || path === "") {
			...
			for (let i = 0; i < parts.length; i++) {
				...
				if (!currentDir.children) {
					throw new Error("Directory has no children map");
				}

				// Check for invalid directories
				const next = currentDir.children[part];
	...
	public async getUniqueName(path: string, name: string): Promise<string> {
		let base = name;
		let ext = "";
