...
type Action =
	...
	| { type: "SET_SHOW_BIOS", payload: boolean };
...
const WindowProvider = ({ children }: { children: ReactNode }) => {
	...
	}, [state.systemProps, currentUser, modifyUserProp]);
	...
	const ctx = useMemo<WindowContextShape>(
		() => ({
			...
			showBios: state.showBios,
			setShowBios: (m) => dispatch({ type: "SET_SHOW_BIOS", payload: m }),
